...
linked_libs = -lc -lpthread -lssl -lcrypto -lcurl -lunit -lvznidd -lhiredis -lhiredis_ssl
...
all: libvznidd.a libhiredis.a libhiredis_ssl.a app

$(INSTALL_INCLUDE_PATH)/jsmn.h:
	$(INSTALL) $(MODULE_PATH)/jsmn/jsmn.h $(INSTALL_INCLUDE_PATH)

$(INSTALL_INCLUDE_PATH)/vzw_nidd.h:
	$(INSTALL) $(MODULE_PATH)/vzw-nidd-sdk/include/vzw_nidd.h $(INSTALL_INCLUDE_PATH)
...
libvznidd.a:
	$(MAKE) -C $(MODULE_PATH)/vzw-nidd-sdk libvznidd.a
	$(INSTALL) $(MODULE_PATH)/vzw-nidd-sdk/libvznidd.a $(INSTALL_LIBRARY_PATH)
...
app: config/vzw_secrets.h $(addprefix $(INSTALL_INCLUDE_PATH)/, jsmn.h vzw_nidd.h $(hiredis_headers)) $(objects)
...
%.o: %.c | libhiredis.a libhiredis_ssl.a
	$(CC) -c $(CFLAGS) -o $@ $<

clean:
	rm -rf $(DIR)/app $(objects) $(DIR)/lib/*.a $(DIR)/include/jsmn.h \
		$(DIR)/include/vzw_nidd.h $(DIR)/include/hiredis
	$(MAKE) -C $(MODULE_PATH)/vzw-nidd-sdk clean
